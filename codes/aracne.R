...
README = function(){
	...
	writeLines("* tfPairEnrich[[1]] is a K x K symetric matrix where the entry [i,i] is zero and the entry [i, j] is")
...
makeGraphs <- function (which = "mods_events", save = FALSE, fName = NULL,
  ...
  # params[[3]]: output directory path
  # e.g., params=list("varNamesVP", "euclidean", "//isilon.c2b2.columbia.edu/ifs/archive/shares/af_lab/GTEx/viper/clustering/per_tissue/")
  ...
  if (which == "viper_mds_per_tissue"){
    if(!is.null(params) && length(params) > 2) {
      ...
      for(i in 1:length(viperMatNames)) {
        ...
        ### plot the MDS
        png(paste0(params[[3]], "clustering_", viperMatNames[i], ".png"),
  ...
  if (which == "regulon_conservation") {
    ...
    plotHeatMap <- function(gene){
      ...
      rd<-as.dist(mat)
      rc<-hclust(rd)
      title = paste("Clustering of nets according to regulon conservation for gene:", gene)
      par(cex.main=.8)
  ...
  if (which == "viper_heatmap"){
    if(!is.null(params) && length(params) > 2) {
      ...
      for(i in 1:length(viperMatNames)) {
        ...
        ### heatmap
        png(paste0(params[[3]], "heatmap_", viperMatNames[i], ".png"),
...
calFet <- function() {
  if(exists("tfPairEnrich")) {
    ...
    sumFet <- function(enrich, digit) {
      
      score <- 0
      minV <- min(enrich[which(enrich[,2] != (-Inf)),2])
      
      if(digit == 2) {
        hubMap <- geneTypeMap[union(which(geneTypeMap[,2] == 1), which(geneTypeMap[,2] == 2)),2]
        names(hubMap) <- geneTypeMap[union(which(geneTypeMap[,2] == 1), which(geneTypeMap[,2] == 2)),1]
      } else {
        hubMap <- geneTypeMap[which(geneTypeMap[,2] == digit),2]
        names(hubMap) <- geneTypeMap[which(geneTypeMap[,2] == digit),1]
      }
      
      for(i in 1:nrow(enrich)){
        if(!is.na(hubMap[as.character(enrich[i,1])])) {
          score <- score + max(minV, enrich[i,2])
        }
      }
    ...
    for(i in 1:(n-1)) {
      for(j in (i+1):n) {
        fet_tf[j,i] <- sumFet(tfPairEnrich[[2]][tfPairEnrich[[1]][j,i]][[1]], 1)
        fet_tfcotf[j,i] <- sumFet(tfPairEnrich[[2]][tfPairEnrich[[1]][j,i]][[1]], 2)
        fet_signal[j,i] <- sumFet(tfPairEnrich[[2]][tfPairEnrich[[1]][j,i]][[1]], 3)
...
calNetProb <- function() {
  if(exists("tfPairEnrich")) {
    ...
    rownames(prob_all) <- rownames(tfPairEnrich[[1]])
    colnames(prob_all) <- colnames(tfPairEnrich[[1]])
    rownames(prob_tf) <- rownames(tfPairEnrich[[1]])
    colnames(prob_tf) <- colnames(tfPairEnrich[[1]])
    rownames(prob_tfcotf) <- rownames(tfPairEnrich[[1]])
    colnames(prob_tfcotf) <- colnames(tfPairEnrich[[1]])
    rownames(prob_signal) <- rownames(tfPairEnrich[[1]])
    colnames(prob_signal) <- colnames(tfPairEnrich[[1]])
    ...
    sumProb <- function(enrich, digit) {
      
      prob <- enrich[,1:2]
      for(i in 1:nrow(prob)) {
        prob[i,2] <- calProb(total_geneNum, enrich[i, 4], enrich[i, 5], enrich[i, 3])
      }
      
      score <- 0
      minV <- min(prob[which(prob[,2] != (-Inf)),2])
      ...
      for(i in 1:nrow(prob)){
        if(!is.na(hubMap[as.character(prob[i,1])])) {
          score <- score + max(minV, prob[i,2])
        }
      }
    ...
    ### summing up the probs
    for(i in 1:(n-1)) {
      for(j in (i+1):n) {
        prob_tf[j,i] <- sumProb(tfPairEnrich[[2]][tfPairEnrich[[1]][j,i]][[1]], 1)
        prob_tfcotf[j,i] <- sumProb(tfPairEnrich[[2]][tfPairEnrich[[1]][j,i]][[1]], 2)
        prob_signal[j,i] <- sumProb(tfPairEnrich[[2]][tfPairEnrich[[1]][j,i]][[1]], 3)
        prob_all[j,i] <- prob_tfcotf[j,i] + prob_signal[j,i]
      }
    }
    
    total_prob <- list(prob_all, prob_tf, prob_tfcotf, prob_signal)
...
saveToRDA <- function (mode = "ARACNe", fName = NULL){
	...
	if (mode == "ARACNe"){
		vars = c(varNames, "varNames", "pairWise", "netSizes", "tfPairEnrich", "tfPairProb", "README")
		if (is.null(fName))
			fName = "aracne.rda"
	}
	else if (mode == "DIGGIT"){
...
oneOffs<- function (which = "freq_mods", params=NULL){
  ...
  #			are treated as 	f they do not exist).
  ...
  #
  # The following params[[]] valuesa are optional: they can either be all ommitted (in which case
