...
# Load the list that maps GO term ids to theis "is_a" ancestors
if (!exists("goTermAncestors"))	
	load("Data/goTermAncestors.rda")
...
# synergy calculation. There is also an option to write the results in an RDA
# binary file. This is useful if te output is intended for computational post
# processing, instead of human inspection. In this case, the MARINa results 
# are appended (as a list variable) to the RDA file.
# 
...
#		"<worksheet> - Synergy". But if 'format' == "rda" (see below) then
#		'worksheet' will be the variable name of the list object used for storing
#		the contents of 'mrs'. In this case, the list contains two members:
#		- <worksheet>[["marina_hubs"]] contains the single-hub enrichment results.
#		- <worksheet>[["marina_synergy"]] contains the MARINa synergy results.
#		the hub enrichment results
# * format:		Specifies if the results will be stored in an Excel or a R
#		binary data file.
# * thresh:		Numeric value between 0 and 1. Only hubs (and hub pairs, for
#		synergy analysis) that clear thins singificance threshold will be saved.
# * thresh_type:	Indicates if the significance threshold value in 'thresh' 
#		applies to the FDR (default) or the unadjusted p-values computed by
#		MARINa.
# *****************************************************************************
writeMarinaResults = function(mrs, file_name, worksheet, regul, format = c("xlsx", "rda"),
		thresh = 1, thresh_type = c("fdr", "pval")){
	...
	# Filter results, based on threshold settings
	if (thresh_type[1] == "fdr")
		keep = (fdr <= thresh)
	else
		keep = (pvals <= thresh)
	fdr = fdr[keep]
	...
	pvals = pvals[keep]
	sizes = sizes[keep]
	ledge = ledge[keep[1:length(ledge)]]
	...
	if (format[1] == "rda"){
		# Make sure the string in 'worksheet' can be used a syntactically
		# variable name
		worksheet = make.names(worksheet)
		tmp_list = list(marina_hubs, marina_synergy)
		names(tmp_list) = c("marina_hubs", "marina_synergy")
		assign(worksheet, tmp_list, envir = globalenv())
		append.Rda(worksheet, file_name)
	}
}
...
# FALSE depending on if the gene gids[i] is a transction factor or not. V is 
# named after the gene IDs of the query genes(s), specifically, names(V) = gids.
# *****************************************************************************
is.tf <- function(gids){
	res = gene.type(gids)
	return(res == rep("TF", length(gids)))
}
...
is.cotf <- function(gids){
	res = gene.type(gids)
	return(res == rep("co-TF", length(gids)))
}
...
is.sign <- function(gids){
	res = gene.type(gids)
	return(res == rep("SIGN", length(gids)))
}
...
gene.type <-function(gids){
	gids = as.entrezId(gids)
	res = rep(NA, length(gids))
	names(res) = gids
	hubs = intersect(gids,names(geneTypeMap))
	if (length(hubs) > 0)
		res[hubs] = geneTypeMap[hubs]
...
#
# Downloaded via: http://www.cookbook-r.com/Graphs/Multiple_graphs_on_one_page_(ggplot2)/
#
# *********************************************************************************
...
#'******************************************************************************
#' Saves a set of variables to a binary RDA file. If the file already exists, it
#' adds the new variables, retaining all other variables aleady saved in the file.
#' Be mindful of performance issues when working with large files as this 
#' method first reads the RDA file in memory, to retrieve the variables there.
#' Also, in the case of variable name collisions (i.e., if the name of a variable
#' to be saved is the same as the name of a variable already saved in the file),
#' the operation aborts with a message.
#' IMPORTANT: this method REQUIRES that the variable names in 'vars' correspond
#' to variables that belong to the global environment.
#'******************************************************************************
#' @title append.Rda
#' @param	vars 	character vector, containing the names of the variables 
#' 		to be saved.
#' @param	file_name	character string, providing the name of the rda file 
#' 		where to store the varables
#' @param	overwrite	boolean specifying what to do if the rda file already
#' 		contains a variable named after a string in 'vars'. If TRUE, the old
#' 		variable will be overwritten. If FALSE, the method will abort with an
#' 		error message
#' @export
####################################################
append.Rda <- function(vars, file_name, overwrite = FALSE) {
	if (!file.exists(file_name))
		save(list = vars, file = file_name)
	else{
		old.objects <- load(file_name, new.env())
		if (length(intersect(old.objects, vars)) > 0)
			if (!overwrite)
				stop("append.Rda:: variables in the file have the same name as those in the argument list.")
		save(list = unique(c(old.objects, vars)), file = file_name)
	}
	return()
}
...
#'******************************************************************************
#' A function to normalize RNA-Seq data with TPM
#' readCount: RNA-Seq rawcounts in a matrix or in a data frame form
#'            Rows are genes and columns are samples
#'            The row names should be Entrez IDs
#' gene_length_RDA_path: The RDA file that contains gene length information
#'                       e.g., //isilon.c2b2.columbia.edu/ifs/archive/shares/af_lab/GTEx/RDA_Files/exon_lengths_hg19.rda
#'                       e.g., //isilon.c2b2.columbia.edu/ifs/archive/shares/af_lab/GTEx/RDA_Files/exon_lengths_hg38.rda
#' ref: The genome reference for the gene length information - either "hg19" or "hg38"
#' filter_thresh: The function filters out genes that have at least one sample
#'                with counts larger than the 'filter_thresh' value
#'                e.g., if the 'filter_thresh' = 1, then it removes genes
#'                that have counts <= 1 across all the samples
#'                if 0, then there will be no filtering
#'******************************************************************************
normalizeRNASEQwithTPM <- function(readCount,
                                   gene_length_RDA_path,
                                   ref="hg19",
                                   filter_thresh=1) {
  
  ### load data
  if(ref == "hg19" && !exists("gene_lengths")) {
    load(as.character(gene_length_RDA_path), envir = globalenv())
    gene_lengths <- get("exon_lengths_hg19", envir = globalenv())
  } else if(ref == "hg38" && !exists("gene_lengths")) {
    load(as.character(gene_length_RDA_path), envir = globalenv())
    gene_lengths <- get("exon_lengths_hg38", envir = globalenv())
  }
  Sys.sleep(3)
  ...
  if(filter_thresh > 0) {
    ### Remove rubbish rows - this will decrease the number of rows
    keep <- apply(readCount, 1, function(r){
      return(sum(r > filter_thresh) > 0)
    })
    readCount <- readCount[keep,]
  }
  
  ### get gene lengths for the given genes
  gene_lens <- gene_lengths[rownames(readCount)]
  
  ### if there is no gene length information, remove those genes out
  keep <- which(!is.na(gene_lens))
  readCount <- readCount[keep,]
  gene_lens <- gene_lens[keep]
  
  ### TPM normalization
  ### normalize based on gene length first
  normCount <- readCount
  for(gene in rownames(normCount)) {
    normCount[gene,] <- normCount[gene,] / gene_lens[gene]
  }
  ### normalize based on read depth
  normCount <- t(t(normCount) * 1e6 / colSums(normCount))
...
#'******************************************************************************
#' The function generates a bar plot that represents sum of each column
#' For example, in case of RNA-Seq read count data, this plot can show
#' read count sum of each sample.
#' You could compare before normalization (mat1) and after normalization (mat2).
#'******************************************************************************
#' @title bar_plot_colSum
#' @param	mat1  first input numeric matrix to print the bar plot	
#' @param	mat2  second input numeric matrix. If Null (defualt),
#'              the function will generate a plot only with the mat1
#' @param	main  plot title at the top of the plot
#' @param xlab  legend for the x-axis
#' @param ylab  legend for the y-axis
#' @param save  boolean value that the plot will be saved in a file or not
#' @param fName plot file name
#' @param width Integer. The width, in pixels, for the plot image
#' @param height  Integer. The height, in pixels, for the plot image
#' @param res Integer. The resolution for the image.
#' @export
####################################################
bar_plot_colSum <- function(mat1, mat2 = NULL, main = "Column_Sums", xlab = "Samples", ylab = "Sums",
                            save = FALSE, fName = "bar_plot_colSum.png",
  ...
  if(!is.null(mat2)) {
    ...
    barplot(apply(mat1, 2, sum), xlab = xlab, ylab = ylab)
    barplot(apply(mat2, 2, sum), xlab = xlab, ylab = ylab)
    mtext(main, outer = TRUE, cex = 1.5)
  } else {
    barplot(apply(mat1, 2, sum), main = main, xlab = xlab, ylab = ylab)
  }
...
#'*********************************************************************************
#' Takes as input a set of GO query terms and maps each one to ancestor terms 
#' related to the query via the "isa" relationship.
#'*********************************************************************************
#' @title	mapGoTermsToAncestors
#' @param 	query_terms		A character vector containing strings that represent GO 
#' 			term ids (e.g., "GO:0000183"). 
#' @param 	go_slim			Either NULL of a character vector of GO term ids.
#' @param	discard_na		If TRUE, then query terms with no ancestors are removed
#' 		from the results object (this is relevant in the case where a non-NULL go_slim
#' 		has been provided; or if there is a type in a query term). Otherwise such 
#' 		terms are mapped to NAs and they are included.
#' @return 	Returns a list L with one entry per query GO term id found in query_terms. 
#' 		The list is named, such that names(L)[i] == query_terms[i]. The list entry L[[i]] 
#' 		is a vector of GO term ids, comprising the parent terms of query_terms[i] in 
#' 		the GO ontology according to the "is a" relationship. If go_slim == NULL, then
#' 		all parent terms are returned. Else, if go_slim is a character vector of GO term
#' 		ids, only parent terms that are found in go_slim are included in L[[i]]. In this 
#' 		case, if a query_terms[i] has no ancestor in go_slim, then L[[i]] = NA. Such 
#' 		NA mappings will be returned only if discard_na == FALSE.
mapGoTermsToAncestors <- function(query_terms, go_slim = NULL, discard_na = FALSE){
	res = goTermAncestors[query_terms]
	if (!is.null(go_slim)){
		res = lapply(res, function(map){
					keep = map %in% go_slim
					if (sum(keep) == 0)
						return(NA)
					else
	...
	names(res) = query_terms
	if (discard_na){
		keep = !sapply(res, function(e){
					return(is.na(e)[1])
				})
		res = res[keep]
	}
